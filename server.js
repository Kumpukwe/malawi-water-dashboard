...
const pool = mysql.createPool({
    host: process.env.RAILWAY_DB_HOST || process.env.LOCAL_DB_HOST || 'localhost',
    user: process.env.RAILWAY_DB_USER || process.env.LOCAL_DB_USER || 'root',
    password: process.env.RAILWAY_DB_PASSWORD || process.env.LOCAL_DB_PASSWORD || '',
    database: process.env.RAILWAY_DB_NAME || process.env.LOCAL_DB_NAME || 'malawi_water',
    port: process.env.RAILWAY_DB_PORT || process.env.LOCAL_DB_PORT || 3306,
...
app.post('/api/add-water-point', authenticateToken, (req, res) => {
    ...
    const query = `INSERT INTO ${tableName} 
        (water_point_id, Name, Type, TA, Functionality_Status, Latitude, Longitude) 
        VALUES (?, ?, ?, ?, ?, ?, ?)`;
...
app.get('/data', (req, res) => {
    ...
    if (district) { query += ' AND TA = ?'; params.push(district); }
...
app.get('/mapdata', (req, res) => {
    ...
    let query = `SELECT water_point_id, Name, Type, Functionality_Status as status, Latitude, Longitude
                 FROM ${tableName} WHERE Latitude IS NOT NULL AND Longitude IS NOT NULL`;
    const params = [];
...
app.get('/districts', (req, res) => {
    ...
    executeQuery(query, [], (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json((results || []).map(r => r.district_name).filter(Boolean));
    });
});
...
app.get('/types', (req, res) => {
    ...
    executeQuery(query, [], (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json((results || []).map(r => r.water_type).filter(Boolean));
    });
});
